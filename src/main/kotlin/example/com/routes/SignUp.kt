...
fun Route.signUp(userService: UserService) {
    post("/signUp") {
        ...
        try {
            if (apiKey != null) {
                val firebaseResponse = firebaseSignUpRequest(credentials, apiKey)
...
private suspend fun firebaseSignUpRequest(credentials: FirebaseRequestBody, apiKey: String): FirebaseResponseBody {
    val url = "https://identitytoolkit.googleapis.com/v1/accounts:signUp"
    HttpClient(CIO) {
        install(Logging)
        ...
        val response: HttpResponse = it.request(url) {
